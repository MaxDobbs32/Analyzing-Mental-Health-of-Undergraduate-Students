...
Note that, before the code block loading in all_years_organized.RData, all blocks of code are set not to run automatically. Running them takes a very long time and only needs to be done once; given that all_years_organized.RData has already been created and stored in this project, running these blocks of code is unnecessary. If you wish to run them, you can do so manually with data_wrangling.qmd opened in RStudio.

As a first step, the code below reads CSV files containing survey data from the Healthy Minds Network and creates R data frames from them. If you wish to run it, you must make sure all data from the Healthy Minds study is in the initial_data subfolder. (See README.md)
...
After the data frames are pulled, the code below saves each of them into an RDS file in the partially_wrangled_data subfolder, where they will be much quicker to retrieve from the hard drive.
...
This code loads the data frames of the form `year_20XX_20XX` from their corresponding RDS files.
